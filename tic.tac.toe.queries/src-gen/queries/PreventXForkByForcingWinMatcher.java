...
/**
 * Generated pattern matcher API of the queries.preventXForkByForcingWin pattern,
 * providing pattern-specific query methods.
 * 
 * <p>Use the pattern matcher on a given model via {@link #on(ViatraQueryEngine)},
 * e.g. in conjunction with {@link ViatraQueryEngine#on(Notifier)}.
 * 
 * <p>Matches of the pattern will be represented as {@link PreventXForkByForcingWinMatch}.
 * 
 * <p>Original source:
 * <code><pre>
 * pattern
 ...
 * @see PreventXForkByForcingWinProcessor
 * @see PreventXForkByForcingWinQuerySpecification
 * 
 */
@SuppressWarnings("all")
public class PreventXForkByForcingWinMatcher extends BaseMatcher<PreventXForkByForcingWinMatch> {
  /**
   ...
   * 
   */
  public static PreventXForkByForcingWinMatcher on(final ViatraQueryEngine engine) throws ViatraQueryException {
    // check if matcher already exists
    PreventXForkByForcingWinMatcher matcher = engine.getExistingMatcher(querySpecification());
    if (matcher == null) {
    	matcher = (PreventXForkByForcingWinMatcher)engine.getMatcher(querySpecification());
    }
    return matcher;
  }
   ...
   * 
   */
  public static PreventXForkByForcingWinMatcher create() throws ViatraQueryException {
    return new PreventXForkByForcingWinMatcher();
  }
  ...
  private final static Logger LOGGER = ViatraQueryLoggingUtil.getLogger(PreventXForkByForcingWinMatcher.class);
   ...
   * 
   */
  private PreventXForkByForcingWinMatcher() throws ViatraQueryException {
   ...
   * @return matches represented as a PreventXForkByForcingWinMatch object.
   * 
   */
  public Collection<PreventXForkByForcingWinMatch> getAllMatches(final Square pSquare, final Engine pEngine, final Color pColor) {
   ...
   * @param pEngine the fixed value of pattern parameter engine, or null if not bound.
   * @param pColor the fixed value of pattern parameter color, or null if not bound.
   * @return a match represented as a PreventXForkByForcingWinMatch object, or null if no match is found.
   * 
   */
  public PreventXForkByForcingWinMatch getOneArbitraryMatch(final Square pSquare, final Engine pEngine, final Color pColor) {
   ...
   * 
   */
  public void forEachMatch(final Square pSquare, final Engine pEngine, final Color pColor, final IMatchProcessor<? super PreventXForkByForcingWinMatch> processor) {
   ...
   * 
   */
  public boolean forOneArbitraryMatch(final Square pSquare, final Engine pEngine, final Color pColor, final IMatchProcessor<? super PreventXForkByForcingWinMatch> processor) {
   ...
   * 
   */
  public PreventXForkByForcingWinMatch newMatch(final Square pSquare, final Engine pEngine, final Color pColor) {
    return PreventXForkByForcingWinMatch.newMatch(pSquare, pEngine, pColor);
  }
   ...
   * 
   */
  public Set<Square> getAllValuesOfsquare(final PreventXForkByForcingWinMatch partialMatch) {
   ...
   * 
   */
  public Set<Engine> getAllValuesOfengine(final PreventXForkByForcingWinMatch partialMatch) {
   ...
   * 
   */
  public Set<Color> getAllValuesOfcolor(final PreventXForkByForcingWinMatch partialMatch) {
  ...
  @Override
  protected PreventXForkByForcingWinMatch tupleToMatch(final Tuple t) {
    try {
    	return PreventXForkByForcingWinMatch.newMatch((Square) t.get(POSITION_SQUARE), (Engine) t.get(POSITION_ENGINE), (Color) t.get(POSITION_COLOR));
  ...
  @Override
  protected PreventXForkByForcingWinMatch arrayToMatch(final Object[] match) {
    try {
    	return PreventXForkByForcingWinMatch.newMatch((Square) match[POSITION_SQUARE], (Engine) match[POSITION_ENGINE], (Color) match[POSITION_COLOR]);
    } catch(ClassCastException e) {
  ...
  @Override
  protected PreventXForkByForcingWinMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return PreventXForkByForcingWinMatch.newMutableMatch((Square) match[POSITION_SQUARE], (Engine) match[POSITION_ENGINE], (Color) match[POSITION_COLOR]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
   ...
   * 
   */
  public static IQuerySpecification<PreventXForkByForcingWinMatcher> querySpecification() throws ViatraQueryException {
