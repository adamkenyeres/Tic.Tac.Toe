...
/**
 * Generated pattern matcher API of the queries.E pattern,
...
public class EMatcher extends BaseMatcher<EMatch> {
   ...
   * 
   */
  public static EMatcher on(final ViatraQueryEngine engine) throws ViatraQueryException {
    // check if matcher already exists
    EMatcher matcher = engine.getExistingMatcher(querySpecification());
    if (matcher == null) {
    	matcher = (EMatcher)engine.getMatcher(querySpecification());
    }
    return matcher;
  }
   ...
   * 
   */
  public static EMatcher create() throws ViatraQueryException {
    return new EMatcher();
  }
  ...
  private final static Logger LOGGER = ViatraQueryLoggingUtil.getLogger(EMatcher.class);
   ...
   * 
   */
  private EMatcher() throws ViatraQueryException {
   ...
   * 
   */
  public Collection<EMatch> getAllMatches(final Square pSource, final Square pTarget) {
   ...
   * 
   */
  public EMatch getOneArbitraryMatch(final Square pSource, final Square pTarget) {
   ...
   * 
   */
  public void forEachMatch(final Square pSource, final Square pTarget, final IMatchProcessor<? super EMatch> processor) {
   ...
   * 
   */
  public boolean forOneArbitraryMatch(final Square pSource, final Square pTarget, final IMatchProcessor<? super EMatch> processor) {
   ...
   * 
   */
  public EMatch newMatch(final Square pSource, final Square pTarget) {
    return EMatch.newMatch(pSource, pTarget);
  }
   ...
   * 
   */
  public Set<Square> getAllValuesOfsource(final EMatch partialMatch) {
   ...
   * 
   */
  public Set<Square> getAllValuesOftarget(final EMatch partialMatch) {
  ...
  protected EMatch tupleToMatch(final Tuple t) {
    try {
    	return EMatch.newMatch((Square) t.get(POSITION_SOURCE), (Square) t.get(POSITION_TARGET));
  ...
  protected EMatch arrayToMatch(final Object[] match) {
    try {
    	return EMatch.newMatch((Square) match[POSITION_SOURCE], (Square) match[POSITION_TARGET]);
    } catch(ClassCastException e) {
  ...
  protected EMatch arrayToMatchMutable(final Object[] match) {
    try {
    	return EMatch.newMutableMatch((Square) match[POSITION_SOURCE], (Square) match[POSITION_TARGET]);
    } catch(ClassCastException e) {
    	LOGGER.error("Element(s) in array not properly typed!",e);
    	return null;
    }
   ...
   * 
   */
  public static IQuerySpecification<EMatcher> querySpecification() throws ViatraQueryException {
